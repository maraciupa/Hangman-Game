 ...
 void play()
{
     ...
     printf("Choose the difficulty level:\n1. Easy (60 seconds) 2. Medium (40 seconds) 3. Hard (25 seconds): ");
     scanf("%d", &difficulty_choice);
     // Setarea timerului si punctajului maxim in functie de dificultate
     int time_limit;
     ...
     int tries = 0;
     double score = 0.0; // scorul obtinut
     double max_score = 0.0; // scorul total posibil
     // Calculam scorul total posibil
     max_score = points_per_letter * word_len;
         ...
         for (int i = 0; i < word_len; i++)
         {
             if (tolower(guessed[i]) == guess)
             {
                 if (current_guess[i] == '_')
                 {
                     ...
                     correct_guesses++;
                     // Adaugam punctaj pentru litera ghicita
                     score = score + points_per_letter;
                     found = 1;
                 }
             }
         }
     ...
     printf("\nYour score: %.2f points out of %.2f", score, max_score);
 }
 int main()
{
     play();
